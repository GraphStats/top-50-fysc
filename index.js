...
function fix() {
    ...
    if (data.autosave == true) {
        ...
        saveInterval = setInterval(saveData2, 1000);
    } else {
...
function restartFadeInMove() {
    document.querySelectorAll('.card').forEach((card, index) => {
        ...
        card.style.animation = `fadeInMove 5s ease-out forwards ${index * 0.01}s`; // Réapplique avec délai progressif
    });
}
...
function observeChanges() {
    ...
    observer.observe(document.getElementById('main'), { childList: true, subtree: true, attributes: true });
}

// Appliquer au chargement et écouter les changements
